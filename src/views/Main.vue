		...
		methods: {
			...
			add(formName) {
				this.$refs[formName].validate((valid) => {
					if(valid){
						if(this.parentNode){
							let newChild   = { id: this.dataList.length+1, label:this.form.name ,children : [] };
							this.dataList.push(newChild);
						}
						if(this.childrenNode){
							let newChild = { id: (this.childrenData.children.length+1)*10+1, label: this.form.name, children: [] };
