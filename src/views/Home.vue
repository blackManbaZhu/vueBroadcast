		...
		<el-col :span="24" class="header">
			...
			<el-col :span="4" class="userinfo">
				<el-dropdown trigger="hover">
					<span class="el-dropdown-link userinfo-inner"><img src="/static/logo.png" />{{sysUserName}}</span>
			...
			<aside :class="collapsed?'menu-collapsed':'menu-expanded'">
				...
				<el-menu :default-active="$route.path" class="el-menu-vertical-demo"  @open="handleopen" @close="handleclose"  unique-opened router>
					<template v-for="(item,index) in $router.options.routes" v-if="!item.hidden">
						...
						<el-menu-item v-if="item.leaf && item.children.length>0" :index="item.children[0].path" :key="index"><i :class="item.icon"></i>{{item.children[0].name}}</el-menu-item>
					</template>
				</el-menu>
				</div>
				<!--导航菜单-折叠后-->
...
<script>
	export default {
		...
		mounted() {
			this.sysUserName = JSON.parse(sessionStorage.getItem('user'));
		}
	}
